...
public:
    ...
    void preencheLista()
    {
        ...
        {
            for(Aresta* a = no->getPrimeiraAresta(); a != nullptr; a = a->getProx())
            {
                lista->insereAresta(no->getId(),a->getId(),a->getPeso());
            }
        }
    }
private:
    Grafo *grafo;
    ListaArestaDuasPontas lista;
    ...
    //função para ordenar
    int binarySearch(ArestaDuasPontas **a, Aresta *item, int low, int high)
    {
        if (high <= low)
        {
